...
for mac in ${mymacs[@]};
do
	out_efi="/tftpboot/grub.cfg-01-${mac//:/-}"
...
device=$(ip addr show | awk '/inet.*brd/{print $NF; exit}')

wget http://${masterIP}/ifcfg-node -O /etc/sysconfig/network-scripts/ifcfg-$device

# Last digit of my IP address
N=$(ip -o addr show dev "$device" | awk '$3 == "inet" {print $4}' | sed -r 's!/.*!!; s!.*\.!!')
